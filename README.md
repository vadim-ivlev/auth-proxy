...
## Роли auth-proxy. Администрирование пользователей и приложений
...
1. Каптча всегда доступна по пути /captcha . Рекомендуется добавлять к пути случайный набор символов:  ?12343214 чтобы избежать кэширования на промежуточных серверах.

2. Поле captcha проверяется в методе login(email, passwod, captcha) ТОЛЬКО ЕСЛИ установлена переменная use_captcha=true И превышено количество допустимых неверных вводов max_attempts=5

3. Чтобы проверить будет ли учитываться капча при попытке входа есть метод is_captcha_required(email), который возвращает {"is_required": true } если выполнены условия перечисленные в педыдущем пункте. Рекомендуется показывать пользователю картинку только после такой проверки.

4. Изображение будет пустым если переменная use_captcha=false
...
Метод `login(... )`, помимо `username` и `password`, имеет входной параметр `pin`, 
который игнорируется если пользователю не требуется вводить пин, или глобальный флаг-требовать-пин выключен.

Узнать, требуется ли пользователю вводить пин, можно вызвав метод:
`is_pin_required( username )`.
