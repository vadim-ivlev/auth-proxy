...
INSERT INTO app (appname, url, description, sign)                 VALUES ('node1'      ,'http://auth-node:3001', 'Работает на продакшн env=prod. Служит для показа заголовков и тела запросов пропущенных через auth-proxy. Express приложение. test0', 'Y');
INSERT INTO app (appname, url, description, sign)                 VALUES ('node2'      ,'http://localhost:3002', 'Работает на компьютере разработчика env=dev. Служит для показа заголовков и тела запросов пропущенных через auth-proxy. Express приложение. test0', 'Y');
